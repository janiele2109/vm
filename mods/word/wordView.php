	...
	if ( $result = $mysqli->query( $query ) )
	{
		while ( $row = mysqli_fetch_row( $result ) )
		{
			echo "<tr class='dynRowWord'>" .
					...
					<td name='word'><span id='span-word-wordId-" . $row[0] . "-wordlistId-" . $row[1] . "' class ='word'>" . $row[0] . "</span></td>
					<td name='wordlist'><span id='span-word-wordlistId-" . $row[1] . "-wordId-" . $row[0] . "' class ='wordlist'>" . $row[1] . "</span></td>
